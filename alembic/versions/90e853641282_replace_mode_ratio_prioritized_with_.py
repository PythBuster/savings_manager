"""replace_mode_ratio_prioritized_with_equal

Revision ID: 90e853641282
Revises: a9c3b883ad20
Create Date: 2025-06-14 11:46:45.243039

"""
from typing import Sequence, Union

import sqlalchemy as sa

from alembic import op

# revision identifiers, used by Alembic.
revision: str = '90e853641282'
down_revision: Union[str, None] = 'a9c3b883ad20'
branch_labels: Union[str, Sequence[str], None] = None
depends_on: Union[str, Sequence[str], None] = None


# Type names
old_type = 'overflowmoneyboxautomatedsavingsmodetype'
new_type = 'overflowmoneyboxautomatedsavingsmodetype_new'

# Table & column – adjust if needed!
table_name = 'app_settings'
column_name = 'overflow_moneybox_automated_savings_mode'

# Enums
new_enum_values = (
    'COLLECT',
    'ADD_TO_AUTOMATED_SAVINGS_AMOUNT',
    'FILL_UP_LIMITED_MONEYBOXES',
    'RATIO',
    'EQUAL',
)

old_enum_values = (
    'COLLECT',
    'ADD_TO_AUTOMATED_SAVINGS_AMOUNT',
    'FILL_UP_LIMITED_MONEYBOXES',
    'RATIO',
    'RATIO_PRIORITIZED',
)


def upgrade():
    # 1. Change all RATIO_PRIORITIZED values to COLLECT (default) so the cast works
    op.execute(
        f"UPDATE {table_name} "
        f"SET {column_name} = 'COLLECT' "
        f"WHERE {column_name} = 'RATIO_PRIORITIZED'"
    )

    # 2. Create new enum type including EQUAL
    op.execute(f"CREATE TYPE {new_type} AS ENUM {new_enum_values}")

    # 3. Remove default value from column
    op.execute(
        f"ALTER TABLE {table_name} "
        f"ALTER COLUMN {column_name} DROP DEFAULT"
    )

    # 4. Cast column to new enum types
    op.execute(
        f"ALTER TABLE {table_name} "
        f"ALTER COLUMN {column_name} "
        f"TYPE {new_type} USING {column_name}::text::{new_type}"
    )

    # 5. Drop the old enum type
    op.execute(f"DROP TYPE {old_type}")

    # 6. Rename new enum type to original type name
    op.execute(f"ALTER TYPE {new_type} RENAME TO {old_type}")

    # 7. Restore default value
    op.execute(
        f"ALTER TABLE {table_name} "
        f"ALTER COLUMN {column_name} SET DEFAULT 'COLLECT'"
    )


def downgrade():
    # 1. Change all EQUAL values to COLLECT (default) so the cast works
    op.execute(
        f"UPDATE {table_name} "
        f"SET {column_name} = 'COLLECT' "
        f"WHERE {column_name} = 'EQUAL'"
    )

    tmp_type = f"{old_type}_tmp"

    # 2. Create enum type without EQUAL
    op.execute(f"CREATE TYPE {tmp_type} AS ENUM {old_enum_values}")

    # 3. Remove default value
    op.execute(
        f"ALTER TABLE {table_name} "
        f"ALTER COLUMN {column_name} DROP DEFAULT"
    )

    # 4. Cast column to old enum types
    op.execute(
        f"ALTER TABLE {table_name} "
        f"ALTER COLUMN {column_name} "
        f"TYPE {tmp_type} USING {column_name}::text::{tmp_type}"
    )

    # 5. Drop old enum type
    op.execute(f"DROP TYPE {old_type}")

    # 6. Rename temporary type back to original name
    op.execute(f"ALTER TYPE {tmp_type} RENAME TO {old_type}")

    # 7. Restore default value
    op.execute(
        f"ALTER TABLE {table_name} "
        f"ALTER COLUMN {column_name} SET DEFAULT 'COLLECT'"
    )

