...
async def test_automated_savings_overflow_moneybox_mode_equal(
    ...
    last_transaction_log = overflow_moneybox_transaction_logs[0]
    expected_description = "Equal-Mode: Automated Savings."
    assert last_transaction_log["description"] == expected_description
...
def create_test_moneyboxes_refusing(overflow_balance: int) -> list[dict[str, Any]]:
    return [
        {  # overflow moneybox
            "id": 1,
            "priority": 0,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            1503,
            OverflowMoneyboxAutomatedSavingsModeType.RATIO,
            {2: 1, 3: 2, 4: None, 5: None},
        ),
        (
            1500,
            3006,
            OverflowMoneyboxAutomatedSavingsModeType.RATIO,
            {2: 1, 3: 1, 4: None, 5: None},
        ),
        # EQUAL
        (
            1500,
...
async def test_calculate_savings_forecast__ratio_and_equal_partially_refusing_overflow_amounts(
    savings_amount: int,
    overflow_balance: int,
    mode: OverflowMoneyboxAutomatedSavingsModeType,
    expected: dict[int, int | None],
    ...
    result = await AutomatedSavingsDistributionService.calculate_savings_forecast(
        moneyboxes=create_test_moneyboxes_refusing(overflow_balance),
        app_settings={
            "is_automated_saving_active": True,
            "savings_amount": savings_amount,
        },
